...
SPEEDTEST_BINARY="$HOME/speedtest"
...
# Function to check if speedtest is installed
check_speedtest() {
    if ! command -v $SPEEDTEST_BINARY &> /dev/null; then
        print_status $RED "Error: speedtest (Ookla) is not installed or not found at $SPEEDTEST_BINARY"
...
get_server_list() {
    ...
    $SPEEDTEST_BINARY -L -f json > "$RESULTS_DIR/servers_${TIMESTAMP}.json" 2>/dev/null
    
    if [ ! -s "$RESULTS_DIR/servers_${TIMESTAMP}.json" ]; then
        print_status $RED "Error: Could not fetch server list"
        exit 1
    fi
    
    # Extract server information from JSON and convert to readable format
    if command -v jq &> /dev/null; then
        jq -r '.servers[] | "\(.id)) \(.name) - \(.location) - \(.country)"' "$RESULTS_DIR/servers_${TIMESTAMP}.json" > "$RESULTS_DIR/servers_${TIMESTAMP}.txt"
    else
        # Fallback if jq is not available - use basic parsing
        grep -o '"id":[0-9]*' "$RESULTS_DIR/servers_${TIMESTAMP}.json" | cut -d':' -f2 | while read id; do
            grep -A 10 -B 10 "\"id\":$id" "$RESULTS_DIR/servers_${TIMESTAMP}.json" | \
            grep -o '"name":"[^"]*"' | head -1 | cut -d'"' -f4 | \
            sed "s/^/${id}) /" >> "$RESULTS_DIR/servers_${TIMESTAMP}.txt"
        done
    fi
    
    if [ ! -s "$RESULTS_DIR/servers_${TIMESTAMP}.txt" ]; then
        print_status $RED "Error: Could not parse server list"
        exit 1
    fi
...
run_speedtest() {
    ...
    if $SPEEDTEST_BINARY -s $server_id -f json > "$output_file" 2>&1; then
...
analyze_results() {
    ...
    for result_file in "$RESULTS_DIR"/speedtest_*_${TIMESTAMP}.txt; do
        if [ -f "$result_file" ]; then
            if command -v jq &> /dev/null && jq -e '.download.bandwidth and .upload.bandwidth' "$result_file" >/dev/null 2>&1; then
                # Extract speeds using jq
                local download=$(jq -r '.download.bandwidth' "$result_file" | awk '{print $1/125000}')  # Convert to Mbps
                local upload=$(jq -r '.upload.bandwidth' "$result_file" | awk '{print $1/125000}')      # Convert to Mbps
            ...
            if ! grep -q "Download\|Upload\|download\|upload" "$result_file"; then
                ((failed_tests++))
            fi
        fi
    done
...
main() {
    ...
    print_status $BLUE "Starting multi-server speedtest (Ookla version)..."
    ...
    print_status $GREEN "\nMulti-server speedtest completed (Ookla version)!"
